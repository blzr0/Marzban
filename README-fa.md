...
- [تنظیمات](#تنظیمات)
  - [پشتیبانی از Hysteria2](#پشتیبانی-از-hysteria2)
  - [فایل‌های Geo (داده‌های مسیریابی)](#فایلهای-geo-دادههای-مسیریابی)
- [داکیومنت](#داکیومنت)
...
- پشتیبانی از پروتکل های **Vmess**, **VLESS**, **Trojan**, **Shadowsocks** و **Hysteria2**
...
|                       فقط برای مستر: تگ‌های inbound (جدا شده با کاما) که پردازش محلی Xray باید به آن‌ها bind شود؛ نودها تحت تأثیر قرار نمی‌گیرند و همیشه کانفیگ کامل را از طریق متغیر `INBOUNDS` خودشان دریافت می‌کنند (پیش‌فرض: بدون فیلتر) |                                                       INBOUNDS                                                       |
...
|                                                                                     فعال کردن کانفیگ سفارشی JSON فقط برای برنامه‌ی Happ (پیش‌فرض: `False`) | USE_CUSTOM_JSON_FOR_HAPP |
|                                                                                     فعال کردن کانفیگ سفارشی JSON فقط برای برنامه‌ی INCY (پیش‌فرض: `False`) | USE_CUSTOM_JSON_FOR_INCY |
|                                             متن پایه استفاده شده در هدر `profile-title` اشتراک، پیش از اموجی و نام کاربری نمایش داده می‌شود (پیش‌فرض: `Subscription`) | SUB_PROFILE_TITLE |
|                                                              لینک پشتیبانی ارسال شده در هدر `support-url` اشتراک (پیش‌فرض: `https://t.me/`) | SUB_SUPPORT_URL |
|                                             فاصله بروزرسانی اشتراک اعلام شده در هدر `profile-update-interval`، بر حسب ساعت (پیش‌فرض: `12`) | SUB_UPDATE_INTERVAL |
|                                                          متن اعلان ارسال شده در هدر `announce` اشتراک، در صورت تنظیم بودن | SUB_ANNOUNCE |
|                                                                                    اموجی نمایش داده شده کنار عنوان پروفایل اشتراک (پیش‌فرض: `🪨`) | SUB_PROFILE_TITLE_EMOJI |
|                             ارائه یک اشتراک جایگزین (استاب) به‌جای کانفیگ واقعی برای کاربران منقضی‌شده یا محدودشده (پیش‌فرض: `False`) | EXPIRED_SUB_ENABLED |
|                       لینکی که درخواست‌های اشتراک کاربران منقضی/محدودشده به آن هدایت می‌شود (در صورت فعال بودن `EXPIRED_SUB_ENABLED` الزامی است) | EXPIRED_SUB_LINK |
|                                پیام‌های جدا شده با `\|` که به‌جای کانفیگ واقعی به کاربران منقضی/محدودشده نمایش داده می‌شود | EXPIRED_SUB_TITLES |
|                        لینک پشتیبانی نمایش داده شده به کاربران منقضی/محدودشده (در صورت خالی بودن، از `SUB_SUPPORT_URL` استفاده می‌شود) | EXPIRED_SUB_SUPPORT_URL |
|                          فاصله بروزرسانی اشتراک اعلام شده به کاربران منقضی/محدودشده، بر حسب ساعت (پیش‌فرض: `1`) | EXPIRED_SUB_UPDATE_INTERVAL |
|                                هدر اعلان (announce) ارسال شده همراه با اشتراک جایگزین برای کاربران منقضی/محدودشده | EXPIRED_SUB_ANNOUNCE |
|                             ارائه یک اشتراک جایگزین به‌جای خطای ۴۰۴ برای توکن معتبر (از نظر امضا) کاربری که دیگر وجود ندارد (پیش‌فرض: `False`) | DELETED_SUB_ENABLED |
|                       لینک استفاده‌شده برای ساخت لینک‌های جایگزین نمایش داده شده به کاربران حذف‌شده (همراه با `DELETED_SUB_TITLES` برای فعال شدن `DELETED_SUB_ENABLED` الزامی است) | DELETED_SUB_LINK |
|                                پیام‌های جدا شده با `\|` که به‌جای کانفیگ واقعی به کاربران حذف‌شده نمایش داده می‌شود | DELETED_SUB_TITLES |
|                        لینک پشتیبانی نمایش داده شده به کاربران حذف‌شده (در صورت خالی بودن، از `SUB_SUPPORT_URL` استفاده می‌شود) | DELETED_SUB_SUPPORT_URL |
|                          فاصله بروزرسانی اشتراک اعلام شده به کاربران حذف‌شده، بر حسب ساعت (پیش‌فرض: `12`) | DELETED_SUB_UPDATE_INTERVAL |
|                                هدر اعلان (announce) ارسال شده همراه با اشتراک جایگزین برای کاربران حذف‌شده | DELETED_SUB_ANNOUNCE |
|                             ارائه یک اشتراک جایگزین به‌جای کانفیگ واقعی برای توکن معتبر (از نظر امضا) که لینکش باطل شده، درحالی‌که کاربر هنوز وجود دارد (پیش‌فرض: `False`) | REVOKED_SUB_ENABLED |
|                       لینک استفاده‌شده برای ساخت لینک‌های جایگزین نمایش داده شده برای لینک‌های باطل‌شده (همراه با `REVOKED_SUB_TITLES` برای فعال شدن `REVOKED_SUB_ENABLED` الزامی است) | REVOKED_SUB_LINK |
|                                پیام‌های جدا شده با `\|` که به‌جای کانفیگ واقعی برای لینک‌های باطل‌شده نمایش داده می‌شود | REVOKED_SUB_TITLES |
|                        لینک پشتیبانی نمایش داده شده برای لینک‌های باطل‌شده (در صورت خالی بودن، از `SUB_SUPPORT_URL` استفاده می‌شود) | REVOKED_SUB_SUPPORT_URL |
|                          فاصله بروزرسانی اشتراک اعلام شده برای لینک‌های باطل‌شده، بر حسب ساعت (پیش‌فرض: `12`) | REVOKED_SUB_UPDATE_INTERVAL |
|                                هدر اعلان (announce) ارسال شده همراه با اشتراک جایگزین برای لینک‌های باطل‌شده | REVOKED_SUB_ANNOUNCE |
|                                             افزودن لینک‌های اضافی به انتهای اشتراک با فرمت v2ray برای کاربران فعال (پیش‌فرض: `False`) | EXTRA_SUB_ENABLED |
|                                                           لینک‌های جدا شده با `\|`، که بدون تغییر (از قبل انکود شده) به انتهای اشتراک کاربران فعال افزوده می‌شوند | EXTRA_SUB_LINKS |
|                                             تگ‌های inbound جدا شده با کاما که کاربر باید حداقل یکی از آن‌ها را داشته باشد تا `EXTRA_SUB_LINKS` دریافت کند؛ خالی یعنی همه کاربران فعال دریافت می‌کنند | EXTRA_SUB_REQUIRED_INBOUND |

> مقدار `EXTRA_SUB_LINKS` و هر مقدار `*_SUB_LINK`/`*_SUB_TITLES` باید در `.env` داخل گیومه قرار بگیرد، مثلاً `EXTRA_SUB_LINKS="vless://...#remark|vless://...#remark"`: کاراکتر `#` بدون گیومه در `.env` شروع یک کامنت است و باقی خط را قطع می‌کند، و لینک‌های اشتراک معمولاً شامل `#remark` و `&param=value` هستند.
>
> `EXTRA_SUB_LINKS` فقط به فرمت‌های اشتراک **v2ray** (لیست ساده لینک‌ها) و **v2ray-json** افزوده می‌شود، و فقط برای کاربرانی که وضعیت‌شان `active` است (نه `on_hold`/`expired`/`limited`/`disabled`). این لینک‌ها هرگز در اشتراک‌های Clash، Clash-Meta، sing-box و Outline ظاهر نمی‌شوند، چون این فرمت‌ها از آبجکت‌های پروکسی پردازش‌شده ساخته می‌شوند نه از لینک‌های خام.

## پشتیبانی از Hysteria2

مرزبان می‌تواند inbound های نیتیو **Hysteria2** را مدیریت کند؛ پسورد هر کاربر مانند سایر پروتکل‌ها تولید و در صورت نیاز جایگزین می‌شود. نکات مهم:

- نیاز به نسخه‌ای از Xray-core دارد که پشتیبانی نیتیو از Hysteria2 داشته باشد؛ از یک نسخه‌ی جدید استفاده کنید - نسخه‌های اولیه‌ی Hysteria2 مشکلات سازگاری UDP/datagram با کلاینت‌های واقعی داشتند.
- فقط با گواهی TLS واقعی کار می‌کند - **REALITY برای این پروتکل پشتیبانی نمی‌شود** (چون این پروتکل بر پایه QUIC است و شبیه‌سازی handshake پروتکل TCP در REALITY اینجا کاربردی ندارد).
- inbound روی **UDP** گوش می‌دهد، نه TCP - پورت را متناسب با آن باز/فوروارد کنید.

نمونه‌ی حداقلی inbound برای `xray_config.json`:
...
اگر از Marzban-node استفاده می‌کنید، متغیر `XRAY_ASSETS_PATH` را روی هر نود به‌صورت صریح تنظیم کنید - نودها فایل‌های Geo پنل اصلی را به‌طور خودکار به ارث نمی‌برند و بدون این تنظیم از فایل‌هایی که در ایمیج خودشان است استفاده می‌کنند.
